...
// ==== Chat: clear history for this key+mode (actually deletes it server-side,
// not just a local view reset) ====
app.post('/api/chat-clear', chatReadLimiter, requireAccessKey, async (req, res) => {
  const mode = String(req.body?.mode || '');
  if (!ENDPOINTS[mode]) return res.status(400).json({ error: 'bad mode' });
  await kvDelete(`chat:${req.accessKey}:${mode}`);
  res.json({ ok: true });
});
...
app.post('/api/chat', requireAccessKey, chatLimiter, async (req, res) => {
  ...
  history.push({ role: 'bot', text: reply, image: IMAGE_MODES.has(mode) });
