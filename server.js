...
const ADMIN_TOTP_SECRET = process.env.ADMIN_TOTP_SECRET || randomBase32Secret();
if (!process.env.ADMIN_TOTP_SECRET) {
  console.warn('⚠ ADMIN_TOTP_SECRET is not set.');
  console.warn(`⚠ Generated one-time 2FA secret for this boot — add it to your authenticator app now: ${ADMIN_TOTP_SECRET}`);
  console.warn('⚠ Set ADMIN_TOTP_SECRET in your environment variables to keep it fixed across restarts.');
}

// TEMPORARY DEBUG — set DEBUG_TOTP=true in Railway Variables to print the
// currently-correct 6-digit code to the logs every 30s, so you can compare
// it against what your phone shows and find the mismatch. Turn this back
// off (delete the variable, or set it to anything other than "true") once
// login works — it's not something that should stay on permanently.
if (process.env.DEBUG_TOTP === 'true') {
  setInterval(() => {
    const counter = Math.floor(Date.now() / 1000 / 30);
    console.log(`[DEBUG_TOTP] server time: ${new Date().toISOString()} — correct code right now: ${totpAt(ADMIN_TOTP_SECRET, counter)}`);
  }, 30000);
}
...
const uploadJson = express.json({ limit: '6mb' }); // only this route needs room for a base64 screenshot
...
app.post('/api/admin-login', adminLoginLimiter, (req, res) => {
  ...
  const totpOk = verifyTotp(ADMIN_TOTP_SECRET, totp);
  res.json({ ok: passwordOk && totpOk });
});
